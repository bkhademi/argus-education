...
//			->where('Date', $date)
	...
	public function update(Request $request, $id)
	{
		...
		// get pending ORM referrals from the date of the iss referral
		$pendingOroomReferrals = Referrals
			::where('Date', $today)
		...
		switch ($request->param) {
			case 'reassign':
				...
				// doing followup in another day.. make a duplicate
				// move ORM referrals from the iss referral date(if any) to '$tomorrow'
				//if ($today->ne(Carbon::today())) {
					...
					$pendingOroomReferrals->update(['Date' => $tomorrow]);

				//} else {// same day.. dont modify other referrals
				//	$referral->update(['ActivityTypeId' => 0, 'RefferalStatus' => 0]);// modify the same one to be re modified again
				//}
				break;
			case 'oss':
